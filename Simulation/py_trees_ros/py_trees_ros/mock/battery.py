...
class Battery:
    ...
    def __init__(self):
        ...
        self.battery_low_pub = rospy.Publisher('dashboard/gohome', Empty, queue_size=1, latch=False)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.battery.percentage < 30 or self.battery.percentage > 50:
                self.battery_publisher.publish(self.battery)
                print(self.battery.percentage)
            if self.battery.percentage < 25:
                self.battery_low_pub.publish(Empty())
            rate.sleep()
